...
export default function ChatApp({ token, onLogout }) {
  ...
  const [gcIncoming, setGcIncoming] = useState(null);
  ...
  // FIX: store groupCall in ref so signal handlers don't get stale closure
  ...
  const onMessage = useCallback((data) => {
    ...
    if (["call_offer", "call_answer", "ice_candidate", "call_end", "call_reject", "call_unavailable"].includes(event)) {
      handleSignalRef.current?.(data); return;
    }
    if (["gc_start", "gc_join", "gc_members", "gc_offer", "gc_answer", "gc_ice", "gc_end", "gc_leave"].includes(data.type)) {
      handleGcSignalRef.current?.(data); return;
    }
  ...
  const {
    callState, startCall, answerCall, endCall, handleSignal,
    localVideoRef, remoteVideoRef,
    quality, setQuality, networkScore, callStats, QUALITY_PRESETS,
  } = useWebRTC(send, currentUser.id);
  handleSignalRef.current = handleSignal;

  // ── Group Subscribe on group select ─────────────────
  useEffect(() => {
    if (!selectedGroup) return;
    send({ type: "group_subscribe", group_id: selectedGroup.id });
    return () => {
      send({ type: "group_unsubscribe", group_id: selectedGroup.id });
    };
  }, [selectedGroup?.id]);
  ...
  // FIX: removed duplicate ontrack/onconnectionstatechange, added await setLocalDescription
  const createGcPeer = useCallback((remoteUserId, stream, groupId, isInitiator) => {
    // Close stale peer if exists
    if (gcPeersRef.current[remoteUserId]) {
      gcPeersRef.current[remoteUserId].close();
    ...
    pc.ontrack = (e) => {
      ...
      setGroupCall(prev => {
        ...
        participants[remoteUserId] = {
          ...(participants[remoteUserId] || { name: `User ${remoteUserId}`, avatar: null }),
          stream: remoteStream,
        };
        return { ...prev, participants, status: "active" };
      });
      // Also directly assign to video element if already mounted
      if (gcRemoteVideosRef.current[remoteUserId]) {
        gcRemoteVideosRef.current[remoteUserId].srcObject = remoteStream;
      }
    };

    pc.onconnectionstatechange = () => {
      console.log(`[GC] peer ${remoteUserId} → ${pc.connectionState}`);
      if (["disconnected", "failed", "closed"].includes(pc.connectionState)) {
        setGroupCall(prev => {
          if (!prev) return prev;
          const participants = { ...prev.participants };
          delete participants[remoteUserId];
          if (Object.keys(participants).length === 0 && prev.status === "active") {
    ...
    if (isInitiator) {
      pc.createOffer()
        .then(async (offer) => {
          await pc.setLocalDescription(offer); // FIX: await
          send({ type: "gc_offer", to: remoteUserId, group_id: groupId, sdp: pc.localDescription });
        })
        .catch(err => console.error("[GC offer error]", err));
    }
    return pc;
  }, [send]);
      ...
      members
        ...
        .forEach(m => {
          ...
          setGroupCall(prev => prev ? {
            ...prev,
            status: "active",
            participants: { ...prev.participants, [m.id]: { stream: null, name: m.display_name || m.username, avatar: m.avatar_url } }
          } : prev);
        });
  ...
  const joinGroupCall = async (incoming) => {
    ...
    try {
      ...
      setGroupCall({
        ...
        status: "active",
        participants: { [incoming.callerId]: { stream: null, name: incoming.callerName, avatar: null } }
      });
      send({ type: "gc_join", group_id: incoming.groupId, joiner_name: currentUser.display_name || currentUser.username });
      // Peer will be created when we receive gc_members or gc_offer from server
    } catch (e) { alert("Camera/mic access failed: " + e.message); }
  };

  const endGroupCall = useCallback(() => {
    const gc = groupCallRef.current;
    if (gc) send({ type: "gc_end", group_id: gc.groupId });
    Object.values(gcPeersRef.current).forEach(pc => pc.close());
    gcPeersRef.current = {};
    if (localGcStreamRef.current) {
      localGcStreamRef.current.getTracks().forEach(t => t.stop());
      localGcStreamRef.current = null;
    }
  ...
  const toggleGcScreen = async () => {
    if (gcScreenSharing) {
      ...
      if (camTrack) Object.values(gcPeersRef.current).forEach(pc => {
        const sender = pc.getSenders().find(s => s.track?.kind === "video");
        if (sender) sender.replaceTrack(camTrack);
      });
      if (localGcVideoRef.current && localGcStreamRef.current) localGcVideoRef.current.srcObject = localGcStreamRef.current;
      ...
      try {
        ...
        Object.values(gcPeersRef.current).forEach(pc => {
          const sender = pc.getSenders().find(s => s.track?.kind === "video");
          if (sender) sender.replaceTrack(screenTrack);
        });
        if (localGcVideoRef.current) localGcVideoRef.current.srcObject = new MediaStream([screenTrack]);
  ...
  // FIX: handleGcSignal uses groupCallRef to avoid stale closure
  const handleGcSignal = useCallback((data) => {
    const gc = groupCallRef.current;

    if (data.type === "gc_start") {
      if (!gc) setGcIncoming({ groupId: data.group_id, groupName: data.group_name, callerId: data.sender_id, callerName: data.caller_name });

    } else if (data.type === "gc_members") {
      // FIX: server sends existing member list on join — create offers to each
      ...
      (data.members || []).forEach(uid => {
        if (uid !== currentUser.id) {
          setGroupCall(prev => prev ? {
            ...prev,
            participants: { ...prev.participants, [uid]: prev.participants[uid] || { stream: null, name: `User ${uid}`, avatar: null } }
          } : prev);
          createGcPeer(uid, stream, data.group_id, true);
        }
      });

    } else if (data.type === "gc_join") {
      if (gc && data.group_id === gc.groupId) {
        setGroupCall(prev => prev ? {
          ...prev,
          participants: { ...prev.participants, [data.sender_id]: { stream: null, name: data.joiner_name, avatar: null } }
        } : prev);
        const stream = localGcStreamRef.current;
      ...
      // FIX: properly await setLocalDescription for answer
      const handleOffer = async () => {
        let pc = gcPeersRef.current[data.sender_id];
        if (!pc && localGcStreamRef.current) {
          pc = createGcPeer(data.sender_id, localGcStreamRef.current, data.group_id, false);
        }
        if (!pc) return;
        await pc.setRemoteDescription(new RTCSessionDescription(data.sdp));
        const ans = await pc.createAnswer();
        await pc.setLocalDescription(ans);
        send({ type: "gc_answer", to: data.sender_id, group_id: data.group_id, sdp: pc.localDescription });
      };
      handleOffer().catch(e => console.error("[GC answer error]", e));

    } else if (data.type === "gc_answer") {
      const pc = gcPeersRef.current[data.sender_id];
      if (pc && pc.signalingState !== "stable") {
        pc.setRemoteDescription(new RTCSessionDescription(data.sdp))
          .catch(e => console.warn("[GC setRemote answer]", e));
      }
      ...
      if (pc && pc.remoteDescription) {
        pc.addIceCandidate(new RTCIceCandidate(data.candidate))
          .catch(e => console.warn("[GC ice add]", e));
      }

    } else if (data.type === "gc_end") {
      if (gc && gc.groupId === data.group_id) { endGroupCall(); }

    } else if (data.type === "gc_leave") {
      ...
      gcPeersRef.current[uid]?.close();
      delete gcPeersRef.current[uid];
    }
  }, [createGcPeer, endGroupCall, send, currentUser.id]);

  handleGcSignalRef.current = handleGcSignal;

  // Local GC video assign when overlay renders
  useEffect(() => {
    if (groupCall && localGcStreamRef.current && localGcVideoRef.current) {
      localGcVideoRef.current.srcObject = localGcStreamRef.current;
    }
  }, [groupCall]);
  ...
  const formatLastSeen = (user) => {
    ...
    return "Last seen " + d.toLocaleDateString("en-LK", { day: "numeric", month: "short", timeZone: "Asia/Colombo" });
  };
  ...
  const renderMsgContent = (msg) => {
    ...
    if (type === "file" && fileUrl) {
      ...
      if (isVideo) return <div><video controls style={{ maxWidth: "100%", maxHeight: 200, borderRadius: 8 }}><source src={fileUrl} /></video></div>;
      return (
  ...
  // ── RemoteGcVideo component (stable ref + srcObject) ─
  const RemoteGcVideo = ({ uid, stream, name }) => {
    const vRef = useRef(null);
    useEffect(() => {
      if (vRef.current && stream) vRef.current.srcObject = stream;
      gcRemoteVideosRef.current[uid] = vRef.current;
      return () => { delete gcRemoteVideosRef.current[uid]; };
    }, [stream, uid]);
    return (
      <div style={{ position: "relative", borderRadius: 16, overflow: "hidden", background: "#2d1b69", aspectRatio: "16/9", display: "flex", alignItems: "center", justifyContent: "center", border: "2px solid rgba(167,139,250,0.3)" }}>
        <video ref={vRef} autoPlay playsInline style={{ width: "100%", height: "100%", objectFit: "cover", display: stream ? "block" : "none" }} />
        {!stream && (
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
            <div style={{ width: 64, height: 64, borderRadius: "50%", background: "#7c3aed", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, color: "#fff", fontWeight: 700 }}>{(name || "?")[0].toUpperCase()}</div>
            <span style={{ color: "#a78bfa", fontSize: 12 }}>Connecting...</span>
          </div>
        )}
        <div style={{ position: "absolute", bottom: 8, left: 10, color: "#e9d5ff", fontSize: 12, fontWeight: 600, background: "rgba(0,0,0,0.4)", borderRadius: 6, padding: "2px 8px" }}>{name || `User ${uid}`}</div>
      </div>
    );
  };
    ...
    <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#ede8f7", height: "100%", width: isMobile ? "100vw" : "auto", minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 16px", background: "#ffffff", borderBottom: "1px solid #2a3942", flexShrink: 0 }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ color: "#6b5b9a", fontSize: 12 }}>{isGroup ? `${chatTarget.member_count || ""} members` : formatLastSeen(chatTarget)}</div>
        </div>
      ...
      <div style={{ display: "flex", height: "100dvh", width: "100vw", background: "#ede8f7", fontFamily: "'Segoe UI', sans-serif", overflow: "hidden" }}>
        ...
        {lightboxImg && (
          <div style={{ position: "fixed", inset: 0, background: "rgba(40,20,80,0.97)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 3000 }} onClick={() => setLightboxImg(null)}>
            <button onClick={() => setLightboxImg(null)} style={{ position: "absolute", top: 16, right: 16, background: "rgba(80,40,140,0.5)", border: "none", borderRadius: "50%", width: 40, height: 40, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}><X size={20} color="#fff" /></button>
        ...
        {/* 1-on-1 Call Overlay — with network indicator + quality picker */}
        {callState && (() => {
          const sigColour = ["#f85149","#f85149","#f0a500","#3fb950","#3fb950"][networkScore] || "#3fb950";
          const sigBars = [1,2,3,4].map(i => (
            <div key={i} style={{ width:4, borderRadius:2, height:6+i*4, background: i<=networkScore ? sigColour:"rgba(255,255,255,0.2)", alignSelf:"flex-end" }}/>
          ));
          const qualityLabel = { auto:"Auto ✨", hd:"HD 📺", sd:"SD 📡", data_saver:"Data Saver 🔋" }[quality] || quality;
          return (
            <div style={{ position:"fixed", inset:0, background:"rgba(30,16,51,0.96)", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", zIndex:2000 }}>
              ...
              <video ref={localVideoRef} autoPlay playsInline muted style={{ position:"absolute", bottom:120, right:20, width:140, borderRadius:12, border:"2px solid rgba(124,58,237,0.6)", display: callState.isVideo && callState.status==="active" ? "block":"none", zIndex:1 }}/>

              {/* Top bar: signal + quality selector (active only) */}
              {callState.status === "active" && (
                <div style={{ position:"absolute", top:16, left:0, right:0, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px", zIndex:2 }}>
                  <div style={{ display:"flex", alignItems:"center", gap:8, background:"rgba(0,0,0,0.45)", borderRadius:20, padding:"6px 12px" }}>
                    <div style={{ display:"flex", alignItems:"flex-end", gap:2, height:20 }}>{sigBars}</div>
                    {callStats?.rtt != null && (
                      <span style={{ color:"rgba(255,255,255,0.7)", fontSize:11 }}>
                        {callStats.rtt}ms{callStats.bitrate != null && ` · ${callStats.bitrate}kbps`}
                      </span>
                    )}
                  </div>
                  <div style={{ display:"flex", gap:6 }}>
                    {QUALITY_PRESETS && Object.entries(QUALITY_PRESETS).map(([key, preset]) => (
                      <button key={key} onClick={() => setQuality(key)}
                        style={{ background: quality===key ? "#7c3aed":"rgba(0,0,0,0.45)", border: quality===key ? "1px solid #a78bfa":"1px solid rgba(255,255,255,0.15)", borderRadius:20, padding:"5px 12px", color: quality===key ? "#fff":"rgba(255,255,255,0.6)", fontSize:11, fontWeight:600, cursor:"pointer", display:"flex", alignItems:"center", gap:4 }}>
              ...
              <div style={{ position:"relative", zIndex:1, display:"flex", flexDirection:"column", alignItems:"center", gap:16 }}>
                {!(callState.isVideo && callState.status==="active") && (
                  <div style={{ width:100, height:100, borderRadius:"50%", background:avatarColor(callState.remoteUser), display:"flex", alignItems:"center", justifyContent:"center", fontSize:40, color:"#fff", fontWeight:700 }}>
                    {avatarLetter(callState.remoteUser)}
                  </div>
                )}
                <div style={{ color:"#e9d5ff", fontSize:22, fontWeight:600, textShadow:"0 2px 8px rgba(0,0,0,0.6)" }}>
                  {callState.remoteUser.display_name || callState.remoteUser.username || `User ${callState.remoteUser.id}`}
                </div>
                <div style={{ color:"#a78bfa", fontSize:14, display:"flex", alignItems:"center", gap:6 }}>
                  {callState.status==="outgoing" && <><Phone size={14}/> Calling...</>}
                  {callState.status==="incoming" && <><Phone size={14}/> Incoming {callState.isVideo?"video":"audio"} call...</>}
                  {callState.status==="active" && <><div style={{ width:7, height:7, borderRadius:"50%", background:"#3fb950" }}/> Connected · {qualityLabel}</>}
                </div>
                <div style={{ display:"flex", gap:24, marginTop:8 }}>
                  {callState.status==="incoming" && (
                    <>
                      <button onClick={answerCall} style={{ width:64, height:64, borderRadius:"50%", background:"#7c3aed", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 16px rgba(124,58,237,0.5)" }}><Phone size={28} color="#fff"/></button>
                      <button onClick={() => endCall(callState.remoteUser.id)} style={{ width:64, height:64, borderRadius:"50%", background:"#dc2626", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 16px rgba(220,38,38,0.5)" }}><X size={28} color="#fff"/></button>
                    </>
                  )}
                  {(callState.status==="outgoing" || callState.status==="active") && (
                    <button onClick={() => endCall(callState.remoteUser.id)} style={{ width:64, height:64, borderRadius:"50%", background:"#dc2626", border:"none", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 4px 16px rgba(220,38,38,0.5)" }}><PhoneOff size={28} color="#fff"/></button>
                  )}
                </div>
              </div>
            </div>
        ...
        {groupCall && (
          <div style={{ position: "fixed", inset: 0, background: "linear-gradient(135deg, #1e1033 0%, #3b1f6e 100%)", display: "flex", flexDirection: "column", zIndex: 2000 }}>
            ...
            <div style={{ flex: 1, display: "grid", gridTemplateColumns: Object.keys(groupCall.participants).length === 0 ? "1fr" : Object.keys(groupCall.participants).length === 1 ? "1fr 1fr" : "repeat(auto-fit, minmax(240px, 1fr))", gap: 8, padding: "0 12px", alignItems: "center" }}>
              ...
              {/* Remote tiles — FIX: use RemoteGcVideo component for stable refs */}
              {Object.entries(groupCall.participants).map(([uid, p]) => (
                <RemoteGcVideo key={uid} uid={uid} stream={p.stream} name={p.name} />
              ))}
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 16, padding: "20px 16px 28px" }}>
              <button onClick={toggleGcMute} style={{ width: 56, height: 56, borderRadius: "50%", background: gcMuted ? "#dc2626" : "rgba(167,139,250,0.2)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {gcMuted ? <MicOff size={24} color="#fff" /> : <Mic size={24} color="#e9d5ff" />}
              </button>
              <button onClick={toggleGcVideo} style={{ width: 56, height: 56, borderRadius: "50%", background: gcVideoOff ? "#dc2626" : "rgba(167,139,250,0.2)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
                {gcVideoOff ? <VideoOff size={24} color="#fff" /> : <Video size={24} color="#e9d5ff" />}
              </button>
              <button onClick={toggleGcScreen} style={{ width: 56, height: 56, borderRadius: "50%", background: gcScreenSharing ? "#7c3aed" : "rgba(167,139,250,0.2)", border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
