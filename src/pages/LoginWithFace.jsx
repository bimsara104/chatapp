...
const STEPS = { PHONE: "phone", OTP: "otp", FACE: "face", USERNAME: "username" };
const PREFIX = "+94";
...
export default function Login({ onAuth }) {
  ...
  const fullPhone = PREFIX + phoneInput.replace(/^0+/, "");
  ...
  async function handleVerifyOTP() {
    ...
    try {
      ...
      setPendingToken(res);
      // Face saved ද check කරන්න
      const savedFace = localStorage.getItem(`face_${fullPhone}`);
      if (savedFace) {
        setFaceMode("verify");
      } else {
  ...
  async function handleFaceSuccess({ embedding }) {
    setLoading(true); setError("");
    try {
      if (faceMode === "register") {
        // Locally save embedding
        localStorage.setItem(`face_${fullPhone}`, JSON.stringify(embedding));
        if (pendingToken?.is_new_user && !pendingToken?.username) {
          setStep(STEPS.USERNAME);
        } else {
          onAuth(pendingToken);
        }
      } else {
        // Locally verify embedding
        const saved = localStorage.getItem(`face_${fullPhone}`);
        if (!saved) {
          setFaceMode("register");
          setError("No face registered. Please register your face.");
          return;
        }
        const savedEmbedding = JSON.parse(saved);
        const similarity = cosineSimilarity(embedding, savedEmbedding);
  ...
  const S = {
    ...
    card: { background: "#161b22", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "16px", padding: "40px 36px", width: "100%", maxWidth: "420px" },
    ...
    error: { fontSize: "13px", color: "#f85149", background: "rgba(248,81,73,0.1)", padding: "8px 12px", borderRadius: "8px" },
    ...
    <div style={S.page}>
      <div style={S.card}>
        ...
        <div style={{ textAlign: "center", marginBottom: "28px" }}>
          ...
          <h1 style={{ fontSize: "22px", fontWeight: "600", color: "#e6edf3", margin: "0 0 4px" }}>ChatApp</h1>
          <p style={{ fontSize: "13px", color: "#8b949e", margin: 0 }}>Login with WhatsApp + Face ID</p>
        </div>
        ...
        {step === STEPS.PHONE && (
          <div style={S.col}>
            ...
            <button
              style={{ ...S.btn, opacity: (loading || phoneInput.length < 7) ? 0.6 : 1 }}
        ...
        {step === STEPS.FACE && (
          <div style={S.col}>
            <div style={{ textAlign: "center", marginBottom: "8px" }}>
              <p style={{ fontSize: "14px", fontWeight: "500", color: "#e6edf3", margin: "0 0 4px" }}>
                {faceMode === "register" ? "📸 Register Your Face" : "🔍 Verify Your Identity"}
              </p>
              <p style={{ fontSize: "12px", color: "#8b949e", margin: 0 }}>
                {faceMode === "register"
                  ? "Your face will be saved for future logins"
                  : "Make sure you're the account owner"}
              </p>
            </div>
            {error && <p style={S.error}>{error}</p>}
            <FaceVerification
              ...
              onFail={() => setError("Face verification failed")}
            />
            <button style={S.btnGray} onClick={() => { setStep(STEPS.OTP); setError(""); }}>
              ← Back
            </button>
          </div>
        )}

        {/* Step 4 - Username (new users) */}
        {step === STEPS.USERNAME && (
          <div style={S.col}>
            ...
            {error && <p style={S.error}>{error}</p>}
            <button
              style={{ ...S.btn, opacity: (loading || !username) ? 0.6 : 1 }}
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", marginTop: "20px" }}>
          ...
          <span style={{ fontSize: "11px", color: "#656d76" }}>WhatsApp OTP + Face ID secured</span>
        </div>
