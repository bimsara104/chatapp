...
export default function MeetingRoom({ meetingId, token }) {
  ...
  const connectWS = useCallback(() => {
    ...
    const wsUrl = `${proto}//${host}/meet-ws/${meetingId}?name=${encodeURIComponent(myNameRef.current)}&uid=${myIdRef.current}`;
  ...
  const handleSignal = useCallback(async (data) => {
    ...
    if (data.type === "meet_joined") {
      ...
      setParticipants(prev => ({ ...prev, [data.uid]: { name: data.name, stream: null } }));
      const pc = stream ? createPeer(data.uid, data.name, stream, false) : new RTCPeerConnection(ICE);
      ...
      data.participants?.forEach(p => {
        if (p.uid !== myIdRef.current) {
          ...
          if (stream) createPeer(p.uid, p.name, stream, true);
        }
      });
    }
