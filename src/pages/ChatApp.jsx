...
export default function ChatApp({ token, onLogout }) {
  ...
  loadRecentChats();
  loadGroups();
  // Poll every 3s as WebSocket backup
  const interval = setInterval(() => {
    loadRecentChats();
    if (selectedUserRef.current) {
      api.getConversation(selectedUserRef.current.id)
  ...
  const onMessage = useCallback((data) => {
    // FIX: event field හෝ type field දෙකෙන්ම route කරන්න
    const eventName = data.event || data.type;
    if (["call_offer","call_answer","ice_candidate","call_end"].includes(eventName)) {
      handleSignalRef.current?.({ ...data, event: eventName });
      return;
    }
  ...
  // FIX: groupCall ref use කරලා stale closure avoid කරන්න
  const groupCallRef = useRef(null);
  useEffect(() => { groupCallRef.current = groupCall; }, [groupCall]);

  const handleGcSignal = useCallback((data) => {
    const currentGroupCall = groupCallRef.current;

    if (data.type === "gc_start") {
      // FIX: already in a call නම් ignore කරන්න
      if (currentGroupCall) return;
      setGcIncoming({
        groupId: data.group_id,
        groupName: data.group_name,
        callerId: data.sender_id,
        // FIX: server ගෙන් caller_name හරිහැටි එනවා දැන්
        callerName: data.caller_name || `User ${data.sender_id}`,
      });

    } else if (data.type === "gc_join") {
      if (currentGroupCall && data.group_id === currentGroupCall.groupId) {
        setGroupCall(prev => prev ? {
          ...prev,
          participants: {
            ...prev.participants,
            [data.sender_id]: { stream: null, name: data.joiner_name || `User ${data.sender_id}`, avatar: null }
          }
        } : prev);
        const stream = localGcStreamRef.current;
        // FIX: stream ready නම් peer create කරන්න
        if (stream) createGcPeer(data.sender_id, stream, data.group_id, true);
      }

    } else if (data.type === "gc_offer") {
      // FIX: peer already exist නම් reuse, නැත්නම් create
      ...
      if (pc) {
        pc.setRemoteDescription(new RTCSessionDescription(data.sdp))
          ...
          .then(ans => {
            ...
            send({ type: "gc_answer", to: data.sender_id, group_id: data.group_id, sdp: ans });
          })
          .catch(e => console.error("gc_offer handle error:", e));
      }
      ...
      if (pc) {
        pc.setRemoteDescription(new RTCSessionDescription(data.sdp))
          .catch(e => console.error("gc_answer handle error:", e));
      }

    } else if (data.type === "gc_ice") {
      const pc = gcPeersRef.current[data.sender_id];
      if (pc && data.candidate) {
        // FIX: remote description ready check
      ...
      if (currentGroupCall && currentGroupCall.groupId === data.group_id) {
        endGroupCall();
      }
    }
  }, [createGcPeer, send, endGroupCall]);
